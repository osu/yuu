$(document).ready(() => {
	...
	var upload = (file) => {
		...
		$(".uploadedFiles").append(`
			<li id="${itemID}">
				...
				<progress class="progress is-primary" value="" max="100" id="${progressID}"></progress>
			</li>
		`);
		$.ajax({
			type: "POST",
	...
	$(window).on("paste", (event) => {
		var files = (event.clipboardData || event.originalEvent.clipboardData).items;
		for (var file = 0; file < files.length; file++) {
		  var item = files[file]
		  if (item.kind === 'file') {
			var blob = item.getAsFile()
			var fileName = "pasted-image." + blob.type.match(/(?:[^\/]*\/)([^;]*)/)[1]
